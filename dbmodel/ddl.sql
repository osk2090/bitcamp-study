...
-- 강의실사진
DROP TABLE IF EXISTS ems_room_photo RESTRICT;
...
-- 강사배정
DROP TABLE IF EXISTS emc_tch_lect RESTRICT;
...
CREATE TABLE ems_stud (
  ...
  del_addr  VARCHAR(255) NULL     COMMENT '상세주소', -- 상세주소
...
ALTER TABLE ems_stud
  MODIFY COLUMN mno INTEGER NOT NULL AUTO_INCREMENT COMMENT '학생번호';
...
-- 강의실사진
ALTER TABLE ems_room_photo
  ADD CONSTRAINT PK_ems_room_photo -- 강의실사진 기본키
    PRIMARY KEY (
      pno -- 사진번호
    );
...
CREATE TABLE ems_appl (
  mno  INTEGER     NOT NULL COMMENT '학생번호', -- 학생번호
  lno  INTEGER     NOT NULL COMMENT '강의번호', -- 강의번호
  rdt  DATE        NOT NULL DEFAULT now() COMMENT '수강신청일', -- 수강신청일
...
-- 강사배정
ALTER TABLE emc_tch_lect
  ADD CONSTRAINT PK_emc_tch_lect -- 강사배정 기본키
    PRIMARY KEY (
      mno, -- 강사번호
      lno  -- 강의번호
    );
...
-- 강의실사진
ALTER TABLE ems_room_photo
  ADD CONSTRAINT FK_ems_room_TO_ems_room_photo -- 강의실 -> 강의실사진
    FOREIGN KEY (
      crno -- 강의실번호
    )
    REFERENCES ems_room ( -- 강의실
...
-- 강사배정
ALTER TABLE emc_tch_lect
  ADD CONSTRAINT FK_ems_tch_TO_emc_tch_lect -- 강사 -> 강사배정
    FOREIGN KEY (
      mno -- 강사번호
    )
...
-- 강사배정
ALTER TABLE emc_tch_lect
  ADD CONSTRAINT FK_ems_lect_TO_emc_tch_lect -- 강의 -> 강사배정
    FOREIGN KEY (
      lno -- 강의번호
    )
    REFERENCES ems_lect ( -- 강의
